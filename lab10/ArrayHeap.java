...
public class ArrayHeap<T> implements ExtrinsicPQ<T> {
    ...
    private void sink(int index) {
        // Throws an exception if index is invalid. DON'T CHANGE THIS LINE.
        validateSinkSwimArg(index);
        if((!inBounds(leftIndex(index)))&&(!inBounds(rightIndex(index))))return;
        boolean lflag = inBounds(leftIndex(index))&&contents[index].myPriority>contents[leftIndex(index)].myPriority;
        boolean rflag = inBounds(rightIndex(index))&&contents[index].myPriority>contents[rightIndex(index)].myPriority;
        if(lflag&&!rflag){
    ...
    public void insert(T item, double priority) {
        ...
        int p = size+1;
        contents[p] = new Node(item, priority);
        size++;
        swim(p);
         ...
         * I'm sb
         *
        List<Boolean> flag = new ArrayList<>(); //true means right,false means left
        int n = size+1;
        while(n!=1){
            if(n%2==1) flag.addFirst(true);
            else if(n%2==0) flag.addFirst(false);
            n=n/2;
        }
        size++;
        int p = 1;
        Iterator<Boolean> iterator = flag.iterator();
        while(iterator.hasNext()){
            boolean goRight = iterator.next();
            if(goRight) p = rightIndex(p);
            if(!goRight) p = leftIndex(p);
        }*/
    }
